var q=(o,e,t)=>{if(!e.has(o))throw TypeError("Cannot "+t)};var c=(o,e,t)=>(q(o,e,"read from private field"),t?t.call(o):e.get(o)),f=(o,e,t)=>{if(e.has(o))throw TypeError("Cannot add the same private member more than once");e instanceof WeakSet?e.add(o):e.set(o,t)};import{a as A}from"./vendor-26fe51b3.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const a of n.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&r(a)}).observe(document,{childList:!0,subtree:!0});function t(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerpolicy&&(n.referrerPolicy=s.referrerpolicy),s.crossorigin==="use-credentials"?n.credentials="include":s.crossorigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function r(s){if(s.ep)return;s.ep=!0;const n=t(s);fetch(s.href,n)}})();var i;class M{constructor(e,t){f(this,i,"https://books-backend.p.goit.global/");this.selectedCategory=e,this.bookId=t}fetchBooksCategoryList(){return A.get(`${c(this,i)}books/category-list`)}fetchBestSellersBooks(){return A.get(`${c(this,i)}books/top-books`)}fetchBooksByCategory(e){return A.get(`${c(this,i)}books/category?category=${e}`)}fetchBookById(e){return A.get(`${c(this,i)}books/${e}`)}}i=new WeakMap;const y="data:image/png;base64,",U="/project-DreamTeam09/assets/amazon-icon2x-8bceb58e.png",B="data:image/png;base64,",Z="data:image/png;base64,",L="data:image/png;base64,",v="/project-DreamTeam09/assets/bookshop-icon2x-d9991f63.png",w=document.querySelector("#allModal");document.querySelector(".categorie-list");document.querySelector(".category__books");const u=document.querySelector(".add-storage-button"),g=document.querySelector(".remove-modal-btn"),S=document.querySelector(".storage-description"),l="storage-data";let E=[],d={};u.addEventListener("click",j);g.addEventListener("click",Y);async function J(o){w.innerHTML="";try{const e=await V(o);return b(),X(e),e}catch(e){throw console.error("Error",e),e}}async function V(o){try{d={};const t=await(await fetch(`https://books-backend.p.goit.global/books/${o}`)).json();return d={book_image:t.book_image,title:t.title,author:t.author,marketAmazon:t.buy_links[0].url,marketAppleBooks:t.buy_links[1].url,marketBookshop:t.buy_links[4].url,list_name:t.list_name,id:t._id},t}catch(e){throw console.error("Error",e),e}}function b(){const o=JSON.parse(localStorage.getItem(l)),e=d.id;if(!o||o.length===0){u.style.display="block",g.style.display="none";return}else o.find(r=>r.id===e)?(u.style.display="none",g.style.display="block"):(u.style.display="block",g.style.display="none")}function X(o){const e=o.book_image,t=o.title,r=o.author,s=o.buy_links[0].url,n=o.buy_links[1].url,a=o.buy_links[4].url,T=`  

  <img src="${e}" alt="Book Image" class="image-about-book-modal">

  <div class="info-modal">

  <h2 class="title-about-book-modal">${t}</h2>

  <p class="author-about-book-modal"> ${r}</p>

  <p class="text-about-book-modal">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Error, iure nam facere exercitationem quibusdam cum in quasi impedit perferendis porro. Vero quos minima doloribus magni corporis beatae ducimus officiis! Rerum?</p>

  <ul class="shop-modal-list"> <li class="shop-modal-item"><a href="${s}" target="_blank"

    > <img

     width="62"

    height="19"

    srcset="

    ${y} 1x,

    ${U} 2x

  "

   src="${y}"

    alt="Amazon"

  /></a></li>

  <li class="shop-modal-item"><a href="${n}" target="_blank"

    > <img

    width="33"

    height="32"

    srcset="

    ${B} 1x,

    ${Z} 2x

  "

   src="${B}"

    alt="AppleBooks"

  /></a></li>

  <li class="shop-modal-item"><a href="${a}" target="_blank"

    > <img

    width="38"

    height="36"

    srcset="

    ${L} 1x,

    ${v} 2x

  "

   src="${L}"

    alt="Book-Shop"

  /></a></li>

</ul>

</div>

  `;w.innerHTML=T}function j(){const o=JSON.parse(localStorage.getItem(l)),e=d;!o||o.length===0?(E.push(e),localStorage.setItem(l,JSON.stringify(E))):(o.push(e),localStorage.setItem(l,JSON.stringify(o))),S.textContent="Сongratulations! You have added the book to the shopping list. To delete, press the button “Remove from the shopping list”.",b()}function Y(){S.textContent="";const o=d.id,e=JSON.parse(localStorage.getItem(l)),t=e.findIndex(r=>r.id===o);e.splice(t,1),localStorage.setItem(l,JSON.stringify(e)),b()}const Q=document.querySelector(".about-book-modal"),H=document.querySelector("#modal-close");document.querySelector(".fire-base-backdrop");const m=document.querySelector(".js-backdrop-modal");function N(o){o.preventDefault();const e=this.getAttribute("data-book-id");Q.classList.remove("is-hidden"),m.classList.remove("is-hidden"),console.log(e),J(e)}function k(){Q.classList.add("is-hidden"),m.classList.add("is-hidden")}H.addEventListener("click",k);document.addEventListener("keydown",function(o){o.key==="Escape"&&k()});m.addEventListener("click",function(o){o.target===m&&k()});const C=document.querySelector(".best-sellers-books"),R=new M;D();async function D(){return await R.fetchBestSellersBooks().then(e=>e.data.map(t=>{const r=t.books.map(s=>`<li class="best-sellers-book js-open-modal" data-book-id="${s._id}">
             <img class="best-sellers-book-img" src="${s.book_image}" alt="${s.title}" />
             <p class="best-sellers-book-title">${s.title}</p>
             <p class="best-sellers-book-author">${s.author}</p>
            //  <p class="overlay">Quick view</p>
</li>`).join("");C.insertAdjacentHTML("beforeend",`<li class="best-seller-category"> 
                <p class="category-title"> ${t.list_name} </p> 
                <ul class="best-sellers-list-in-category">
                ${r} 
                </ul> 
                <button type="button" class="best-sellers-see-more-button" data-button-category-name="${t.list_name}">See more</button>
                    </li>`)}).join("")).then(()=>{document.querySelectorAll(".js-open-modal").forEach(function(t){t.addEventListener("click",N)})}).catch(e=>{console.log(e.message),C.innerHTML="No information"})}const O={btn:document.querySelector(".scrollBtn"),show(){this.btn.classList.remove("scrollBtn-hide")},hide(){this.btn.classList.add("scrollBtn-hide")},addEventListener(){this.btn&&(window.addEventListener("scroll",()=>{(window.scrollY||document.documentElement.scrollTop)>100?this.show():this.hide()}),this.btn.onclick=()=>{window.scrollTo({top:0,behavior:"smooth"})})}};O.addEventListener();const F=document.querySelector(".category");function P(o){o.sort((t,r)=>t.list_name.localeCompare(r.list_name));const e=o.map(t=>{const{list_name:r}=t;return`<li><a class='genre' href="#" target="_self">${r}</a></li>`}).join("");F.insertAdjacentHTML("beforeend",e)}function x(){new M().fetchBooksCategoryList().then(e=>{const t=e.data;console.log(t),P(t)}).catch(e=>{console.error(e)})}x();const h=document.querySelector(".header-modal"),W=document.querySelector(".modal-open-btn"),I=document.querySelector("body"),p=document.querySelector(".open-icon");console.dir(p.innerHTML);W.addEventListener("click",G);function G(o){if(h.classList.contains("hidden-modal")){h.classList.remove("hidden-modal"),I.classList.add("stop-scrolling"),p.innerHTML='<use href="./img/sprit.svg#x-close"/>';return}else h.classList.add("hidden-modal"),I.classList.remove("stop-scrolling"),p.innerHTML='<use href="./img/sprit.svg#icon-burger"/>'}const z=document.querySelector(".js-home-link"),$=document.querySelector(".js-home-link"),_=location.pathname;_==="/project-DreamTeam09/home.html"?z.classList.add("active"):$.classList.add("current");const ee=document.querySelector(":root"),K=document.querySelector("#themeSwitch");K.addEventListener("change",oe);function oe(){event.preventDefault(),ee.classList.toggle("dark-theme",K.checked)}
//# sourceMappingURL=dark-theme-f655f948.js.map
