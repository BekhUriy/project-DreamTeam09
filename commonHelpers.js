var Z=(o,e,t)=>{if(!e.has(o))throw TypeError("Cannot "+t)};var c=(o,e,t)=>(Z(o,e,"read from private field"),t?t.call(o):e.get(o)),f=(o,e,t)=>{if(e.has(o))throw TypeError("Cannot add the same private member more than once");e instanceof WeakSet?e.add(o):e.set(o,t)};/* empty css                      */import{a as A}from"./assets/vendor-26fe51b3.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const a of i.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&r(a)}).observe(document,{childList:!0,subtree:!0});function t(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerpolicy&&(i.referrerPolicy=s.referrerpolicy),s.crossorigin==="use-credentials"?i.credentials="include":s.crossorigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function r(s){if(s.ep)return;s.ep=!0;const i=t(s);fetch(s.href,i)}})();var n;class M{constructor(e,t){f(this,n,"https://books-backend.p.goit.global/");this.selectedCategory=e,this.bookId=t}fetchBooksCategoryList(){return A.get(`${c(this,n)}books/category-list`)}fetchBestSellersBooks(){return A.get(`${c(this,n)}books/top-books`)}fetchBooksByCategory(e){return A.get(`${c(this,n)}books/category?category=${e}`)}fetchBookById(e){return A.get(`${c(this,n)}books/${e}`)}}n=new WeakMap;const B=document.querySelector(".best-sellers-books"),q=new M;J();async function J(){return await q.fetchBestSellersBooks().then(e=>e.data.map(t=>{const r=t.books.map(s=>`<li class="best-sellers-book" data-book-id="${s._id}">
             <img class="best-sellers-book-img" src="${s.book_image}" alt="${s.title}" />
             <p class="best-sellers-book-title">${s.title}</p>
             <p class="best-sellers-book-author">${s.author}</p>
</li>`).join("");B.insertAdjacentHTML("beforeend",`<li class="best-seller-category"> 
                <p class="category-title"> ${t.list_name} </p> 
                <ul class="best-sellers-list-in-category">
                ${r} 
                </ul> 
                <button type="button" class="best-sellers-see-more-button" data-button-category-name="${t.list_name}">See more</button>
                    </li>`)}).join("")).catch(e=>{console.log(e.message),B.innerHTML="No information"})}const v={btn:document.querySelector(".scrollBtn"),show(){this.btn.classList.remove("scrollBtn-hide")},hide(){this.btn.classList.add("scrollBtn-hide")},addEventListener(){this.btn&&(window.addEventListener("scroll",()=>{(window.scrollY||document.documentElement.scrollTop)>100?this.show():this.hide()}),this.btn.onclick=()=>{window.scrollTo({top:0,behavior:"smooth"})})}};v.addEventListener();const V=document.querySelector(".category");function N(o){o.sort((t,r)=>t.list_name.localeCompare(r.list_name));const e=o.map(t=>{const{list_name:r}=t;return`<li><a class='genre' href="#" target="_self">${r}</a></li>`}).join("");V.insertAdjacentHTML("beforeend",e)}function X(){new M().fetchBooksCategoryList().then(e=>{const t=e.data;console.log(t),N(t)}).catch(e=>{console.error(e)})}X();const h=document.querySelector(".header-modal"),Y=document.querySelector(".modal-open-btn"),y=document.querySelector("body"),p=document.querySelector(".open-icon");console.dir(p.innerHTML);Y.addEventListener("click",H);function H(o){if(h.classList.contains("hidden-modal")){h.classList.remove("hidden-modal"),y.classList.add("stop-scrolling"),p.innerHTML='<use href="./img/sprit.svg#x-close"/>';return}else h.classList.add("hidden-modal"),y.classList.remove("stop-scrolling"),p.innerHTML='<use href="./img/sprit.svg#icon-burger"/>'}const R=document.querySelector(":root"),w=document.querySelector("#themeSwitch");w.addEventListener("change",j);function j(){event.preventDefault(),R.classList.toggle("dark-theme",w.checked)}const S=document.querySelector(".about-book-modal"),D=document.querySelector("#modal-close");document.querySelector(".fire-base-backdrop");const m=document.querySelector(".js-backdrop-modal");function O(){S.classList.remove("is-hidden"),m.classList.remove("is-hidden")}function b(){S.classList.add("is-hidden"),m.classList.add("is-hidden")}D.addEventListener("click",b);document.addEventListener("keydown",function(o){o.key==="Escape"&&b()});m.addEventListener("click",function(o){o.target===m&&b()});const L="data:image/png;base64,",F="/project-DreamTeam09/assets/amazon-icon2x-8bceb58e.png",E="data:image/png;base64,",x="data:image/png;base64,",C="data:image/png;base64,",P="/project-DreamTeam09/assets/bookshop-icon2x-d9991f63.png",Q=document.querySelector("#allModal"),W=document.querySelector(".categorie-list"),G=document.querySelector(".category__books"),u=document.querySelector(".add-storage-button"),g=document.querySelector(".remove-modal-btn"),K=document.querySelector(".storage-description"),l="storage-data";let I=[],d={};u.addEventListener("click",ee);g.addEventListener("click",oe);G.addEventListener("click",T);W.addEventListener("click",T);function T(o){if(o.target.nodeName==="BUTTON"||o.target.nodeName==="UL"||o.target.nodeName==="DIV"||o.target.nodeName==="H3")return;const e=o.target.closest("li").id;O(),z(e)}async function z(o){Q.innerHTML="";try{const e=await $(o);return k(),_(e),e}catch(e){throw console.error("Error",e),e}}async function $(o){try{d={};const t=await(await fetch(`https://books-backend.p.goit.global/books/${o}`)).json();return d={book_image:t.book_image,title:t.title,author:t.author,marketAmazon:t.buy_links[0].url,marketAppleBooks:t.buy_links[1].url,marketBookshop:t.buy_links[4].url,list_name:t.list_name,id:t._id},t}catch(e){throw console.error("Error",e),e}}function k(){const o=JSON.parse(localStorage.getItem(l)),e=d.id;if(!o||o.length===0){u.style.display="block",g.style.display="none";return}else o.find(r=>r.id===e)?(u.style.display="none",g.style.display="block"):(u.style.display="block",g.style.display="none")}function _(o){const e=o.book_image,t=o.title,r=o.author,s=o.buy_links[0].url,i=o.buy_links[1].url,a=o.buy_links[4].url,U=`  

  <img src="${e}" alt="Book Image" class="image-about-book-modal">

  <div class="info-modal">

  <h2 class="title-about-book-modal">${t}</h2>

  <p class="author-about-book-modal"> ${r}</p>

  <p class="text-about-book-modal">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Error, iure nam facere exercitationem quibusdam cum in quasi impedit perferendis porro. Vero quos minima doloribus magni corporis beatae ducimus officiis! Rerum?</p>

  <ul class="shop-modal-list"> <li class="shop-modal-item"><a href="${s}" target="_blank"

    > <img

     width="62"

    height="19"

    srcset="

    ${L} 1x,

    ${F} 2x

  "

   src="${L}"

    alt="Amazon"

  /></a></li>

  <li class="shop-modal-item"><a href="${i}" target="_blank"

    > <img

    width="33"

    height="32"

    srcset="

    ${E} 1x,

    ${x} 2x

  "

   src="${E}"

    alt="AppleBooks"

  /></a></li>

  <li class="shop-modal-item"><a href="${a}" target="_blank"

    > <img

    width="38"

    height="36"

    srcset="

    ${C} 1x,

    ${P} 2x

  "

   src="${C}"

    alt="Book-Shop"

  /></a></li>

</ul>

</div>

  `;Q.innerHTML=U}function ee(){const o=JSON.parse(localStorage.getItem(l)),e=d;!o||o.length===0?(I.push(e),localStorage.setItem(l,JSON.stringify(I))):(o.push(e),localStorage.setItem(l,JSON.stringify(o))),K.textContent="Сongratulations! You have added the book to the shopping list. To delete, press the button “Remove from the shopping list”.",k()}function oe(){K.textContent="";const o=d.id,e=JSON.parse(localStorage.getItem(l)),t=e.findIndex(r=>r.id===o);e.splice(t,1),localStorage.setItem(l,JSON.stringify(e)),k()}
//# sourceMappingURL=commonHelpers.js.map
